...
function AuctionOffersMoreInfo({ data }) {
    const { acceptNFTAuctionPreviousBid, handleMetamaskError ,isWalletConnected,isWrongWalletConnected} = useContractService()
    ...
    const getFixedOffersData = async () => {
        try {
            ...
            const result = await get(`myAuctionBidList?auctionIndex=${data.auctionIndex}`)
            setOffersData(result.data.nftAuctionBidOffers)
    ...
    async function acceptOfferForAuctionSaleNFT(items) {
        const { bidder } = items
        ...
        try {
            ...
            const result = await acceptNFTAuctionPreviousBid(data.auctionIndex, bidder)
                ...
                <>
                    <div className="flex gap-2 justify-center">
                        <Tooltip title="Accept" arrow>
                            <button className="rounded-sm truncate" onClick={() => acceptOfferForAuctionSaleNFT(item)}>
...
export default AuctionOffersMoreInfo