import React from 'react'
import { Link } from 'react-router-dom'

function SetUpMFA() {
    return (
        <div>
            <img src="/assets/imgs/logo.png" className='Logo' alt="" style={{ border: "0" }} />

            <div className="w-full max-w-sm maincard  m-auto  shadow  p-4"  >
                <div className="">
                    <h1 className='mb-3'  >Set up MFA</h1>
                    <p className='text-left mb-3'  ><b>Install an authenticator appplication on your mobile</b></p>
                    <p className='text-left mb-5' >Select and download an authenticator (like Google Authenticator) from your application store </p>
                    <p className='text-left mb-5' ><b>Open your authenticator application</b></p>
                    <p className='text-left' >Scan the QR code below, and enter the verification code that is displayed in your application.</p>
                    <p><b>The verification code expires in 5 minutes. </b></p>
                    <img src="data:image/png;base64," alt="" style={{ width: "50%", margin: "10px auto" }} />
                    <div className="mb-5">
                        <label htmlFor="email" className="block mb-2 text-sm font-light  text-left inputlabel" >Verification Code</label>
                        <input type="number" id="email" className="logininput" autoComplete="off" placeholder="Enter Verification Code" required />
                    </div>
                    <div className='flex justify-center mt-5' >
                        <Link to="/" className='createsubmitbutton' ><button type="submit" className="p-0 flex  gap-2 m-auto justify-center">Can't scan ?</button></Link>
                        <Link to="/Dashboard" className='createsubmitbutton' ><button type="submit" className="p-0 m-auto flex gap-2 justify-center w-full">Verify</button></Link>
                    </div>
                    <p className=' text-center my-5'>Important Information about legal, Ts & Cs <br /> Copyrights Etc</p>


                </div>

            </div>

        </div>
    )
}

export default SetUpMFA
